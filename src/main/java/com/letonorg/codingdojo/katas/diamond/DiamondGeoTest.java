...
class DiamondGeoTest {
    ...
    void should_create_diamond_for_A() {
        assertEquals("A", normalize(DiamondGeo.makeDiamond('A')));
    }
    ...
    void should_create_diamond_for_B() {
        String expected = """
               ...
               B B
                A
                """;
        assertEquals(normalize(expected), normalize(DiamondGeo.makeDiamond('B')));
    }
        ...
        String expected = """
               ...
               C   C
                B B
                 A
                """;
        assertEquals(normalize(expected), normalize(DiamondGeo.makeDiamond('C')));
    }
               ...
               D     D
                C   C
                 B B
                  A
                """;
        assertEquals(normalize(expected), normalize(DiamondGeo.makeDiamond('D')));
    }

    @Test
    void should_have_correct_number_of_lines() {
        // For letter N: lines = 2*(N-'A') + 1
        assertEquals(1, linesOf(DiamondGeo.makeDiamond('A')));
        assertEquals(3, linesOf(DiamondGeo.makeDiamond('B')));
        assertEquals(5, linesOf(DiamondGeo.makeDiamond('C')));
        assertEquals(7, linesOf(DiamondGeo.makeDiamond('D')));
        assertEquals(51, linesOf(DiamondGeo.makeDiamond('Z')));
    }

    @Test
    void should_be_vertically_symmetric() {
        String diamond = DiamondGeo.makeDiamond('F');
    ...
    void should_be_horizontally_symmetric_on_each_line() {
        String diamond = DiamondGeo.makeDiamond('G');
    ...
    void should_have_A_only_on_first_and_last_line() {
        String diamond = normalize(DiamondGeo.makeDiamond('E'));
        String[] lines = diamond.split("\n");
    ...
    void should_have_each_letter_twice_except_A() {
        String diamond = normalize(DiamondGeo.makeDiamond('H'));
